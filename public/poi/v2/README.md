...
Первая версия в родительской папке сохранена целиком. Приложение использует вторую версию по умолчанию, файлы доступны по пути `/poi/v2/…`. Кнопка «Точки интереса» переключает SVG-значки и эмоджи независимо от кнопки «Гексы».
...
Макет использует текущий тайл `Lowland_deciduous_forest.png`, геометрию и границы гекса из приложения. Корона v2 показана без изменений, по центру, в размере 24 × 24 на гексе высотой 56. Этот размер используется для одиночного SVG в приложении; в режиме эмоджи сохранён размер шрифта 16. Слева увеличение ×4, справа ×2. Фоновый тайл встроен в SVG примера для автономного просмотра.
