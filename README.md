...
- В панели информации отображаются:
  ...
  - целевой размер;
  - фактический размер.
- Реки всегда строятся как связный `vertexPath` строго по рёбрам hex-grid.
- Одно river-edge (нормализованный `edgeKey`) нельзя использовать дважды ни в каком направлении.
- Если новый регион не касается endpoint существующих рек, создаётся новая река между двумя `candidate boundary vertices`.
- Если новый регион касается ровно одного endpoint, река продолжается от него до `candidate boundary vertex` на другой стороне региона.
- Если новый регион касается двух (или более) endpoint-ов, приоритет — соединить две примыкающие реки через регион.
- Внутренние вершины старых рек не используются как точки продолжения.
