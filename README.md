...
- Река строится как связный путь по рёбрам гексов и пересекает регион от одной стороны к другой.
- Новый регион может продолжить существующую реку только от её конечной вершины. Внутренние вершины реки не используются для продолжения.
- Если регион не продолжает существующую реку, новая река строится от candidate boundary vertex до другой candidate boundary vertex, то есть входит в регион из зоны кандидатов и выходит обратно в зону кандидатов.
- Линия реки визуально продолжается к внешним вершинам через соседние candidate-гексы.
