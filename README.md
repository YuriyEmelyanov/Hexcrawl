...
- Концы простой реки выбираются из exposed boundary vertices — вершин внешнего контура региона.
- Новая попытка строит реку от boundary edge до boundary edge. Если такой path не найден, используется старая безопасная логика.
