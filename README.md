...
3. Размер каждого региона определяется двухшаговой fate-процедурой:
   - бросок масштаба: `8dF`, где каждый dF считается как палочки (`- = 0`, `пусто = 1`, `+ = 2`);
   - `X = 1 + sticks(8dF)`;
   - бросок влияния: `XdF` по тем же палочкам `0/1/2`;
...
- В панели информации отображаются:
  - значения броска масштаба (8dF);
  - палочки масштаба и рассчитанный масштаб `X`;
  - значения броска влияния (XdF);
  - палочки влияния и итоговый расчёт размера;
  - целевой размер;
  - фактический размер.
